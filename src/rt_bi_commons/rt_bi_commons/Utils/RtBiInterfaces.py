...
class RtBiInterfaces:
	...
	class TopicNames(Enum):
		...
		RT_BI_EMULATOR_MAP_REGION = "/__rt_bi_emulator/map"
	...
	@staticmethod
	def createDynamicMapRegionPublisher(node: RtBiNode, callbackFunc: Callable = lambda: None, intervalSecs: float = nan) -> tuple[Publisher, Timer | None]:
		return RtBiInterfaces.createSpacePublisher(node, RtBiInterfaces.TopicNames.RT_BI_EMULATOR_MAP_REGION, callbackFunc, intervalSecs)

	@staticmethod
	def subscribeToDynamicMapRegions(node: RtBiNode, callbackFunc: Callable[[Msgs.RtBi.RegularSpaceArray], None]) -> None:
		RtBiInterfaces.subscribeToSpace(node, RtBiInterfaces.TopicNames.RT_BI_EMULATOR_MAP_REGION, callbackFunc)
