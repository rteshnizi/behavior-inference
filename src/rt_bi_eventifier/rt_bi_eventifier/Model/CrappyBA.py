...
class CrappyBehaviorAutomaton(nx.DiGraph):
	...
	def __updateStateLabel(self, state: str) -> str:
		Ros.Log(f"Updating label of {state}.", severity=LoggingSeverity.ERROR)
		cols: list[str] = []
	...
	def __propagateTokens(self, fromState: str, toState: str, iGraph: MetricIGraph) -> None:
		...
		while len(tokens) > 0:
			...
			# BFS
			Ros.Log(f"Path {token['id']}", token['path'], severity=LoggingSeverity.ERROR)
			...
			for destination in extensions:
				...
				Ros.Log(f"Destination {destination}", [(p, ps[p]) for p in ps], severity=LoggingSeverity.ERROR)
				if iGraph.satisfies(destination, statement):
					self.__addToken(toState, newToken)
					if toState in self.__accepting:
						Ros.Log(f"ACCEPTING {newToken['id']}", newToken["path"], severity=LoggingSeverity.ERROR)
				else:
	...
	def evaluate(self, iGraph: MetricIGraph) -> None:
		...
		"""
		Ros.Log(120 * f"┬", severity=LoggingSeverity.ERROR)
		totalTokens = 0
		...
		Ros.Log(f"Evaluating tokens of {self.name}.", severity=LoggingSeverity.ERROR)
		...
		while len(statesToUpdate) > 0:
			...
			Ros.Log(f"From State {fromState}.", severity=LoggingSeverity.ERROR)
			Ros.Log(f"BEFORE REDUCTION has {len(self.states[fromState]['tokens'])} tokens.", severity=LoggingSeverity.ERROR)
			self.reduceUncertainty(fromState, iGraph)
			Ros.Log(f"AFTER REDUCTION has {len(self.states[fromState]['tokens'])} tokens.", severity=LoggingSeverity.ERROR)
			for toState in self[fromState]:
				...
				Ros.Log(f"To State {toState}", severity=LoggingSeverity.ERROR)
