...
class Renderer(Node):
	...
	def init(self):
		...
		"""
		super().__init__("saBil_renderer")
		Renderer.TRANSLATION_X = 0
		Renderer.TRANSLATION_Y = 0
		Renderer.SCALE = 1
		self.__publisher = self.create_publisher(Marker, "visualization_marker", 10)

	def _translateCoords(coord):
		c = [Renderer.SCALE * (coord[0] + Renderer.TRANSLATION_X), Renderer.SCALE * ((coord[1]) + Renderer.TRANSLATION_Y)]
		return c
	...
	def CreateCircle(canvas, centerX, centerY, radius, outline, tag, fill="", width=1):
		...
		marker.ns = "bil"
		marker.id
		c = Renderer._translateCoords([centerX, centerY])
		centerX = c[0]
		centerY = c[1]
	...
	def CreatePolygon(canvas, coords, outline, fill, width, tag, hashFill=False, hashDensity=25):
		...
		coords = [Renderer._translateCoords(c) for c in coords]
	...
	def CreateText(canvas, coords, text, tag, color="Black", fontSize=10):
		...
		"""
		coords = Renderer._translateCoords(coords)
