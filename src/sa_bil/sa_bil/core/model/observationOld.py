...
class ObservationOld:
	...
	def clear(self, canvas):
		...
		for circleId in self._circleIds:
			Renderer.RemoveShape(canvas, circleId)
		self._circleIds = []
	...
	def render(self, canvas):
		...
		for p in self.sensed.poses:
			self._circleIds.append(Renderer.CreateCircle(canvas, p.x, p.y, radius=5, outline=self.trajectory.MARKING_COLOR, fill=self.trajectory.MARKING_COLOR, tag="%s-%.1f" % (self.trajectory.name, p.time)))
